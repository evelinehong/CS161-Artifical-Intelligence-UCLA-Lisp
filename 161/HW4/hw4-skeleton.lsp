...
;
(defun at-most-one-color-helper (n c i k)
  (cond ((> i k) ())
        (t (append (list (list (- (node2var n c k)) (- (node2var n i k)))) (at-most-one-color-helper n c (+ i 1) k)))
  )
)

(defun at-most-one-color (n c k)
  (cond ((= c k) nil)
        (t (append (at-most-one-color-helper n c (+ c 1) k) (at-most-one-color n (+ c 1) k)))
  )
)
...
(defun generate-edge-clauses-helper (a b c k)
  (cond ((> c k) '())
        (t (append (list (list (- (node2var a c k)) (- (node2var b c k)))) (generate-edge-clauses-helper a b (+ c 1) k)))
  )
)
